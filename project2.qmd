...
```{r}
imp <- mice(df.proj2, m = 5, method = "pmm", seed = 1)
...
df.imp$Var_BA <- data_with_interactions$Var * data_with_interactions$BA

ncol=123#main effect and all possible interaction
...
coef.all <- matrix(ncol = 5, nrow = 29) #5 imputation, 29 coeffs
...
for(m in 1:5){
  ...
  coef.all[,m]<-as.vector(coef.iter)[-1]#remove intercept term
...
#training
scores <- x1 %*% mod_lasso_scale
mod1<-glm(y1~scores, family= binomial())
pred1 <- predict(mod1, tb_south_africa, type = "response")#convert to probabilities
...
#testing
x2 <- model.matrix(tb ~ ., tb_uganda)[, -1]
y2 <- tb_uganda$tb
scores <- x2 %*% mod_lasso_scale
mod2<-glm(y2~scores, family= binomial())
pred2 <- predict(mod2, tb_uganda, type = "response")
...
```{r}
df.mice <- mice(df.proj2, meth='pmm', maxit = 10, seed=500, print=F)
#same number of observations per treatment

#mice instruction: https://www.youtube.com/watch?v=ghmU7nodhSM
# reg.fit<-with(df.mice,lm(y~x1+x2))
# pool.fit<-pool(reg.fit)
# summary(pool.fit)
# pool.r2<-pool.r.squared(reg.fit)
```
...
#three way interaction??? BASC*varenicline*site or two way: BASC*varenicline

#unbalanced data, bootstrap or cv??? inverse probability weighting (causal lecture4)???
```
