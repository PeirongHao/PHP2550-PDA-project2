...
legend(1.5, 0.95, legend=c("Derivation data", "Validation data"),
...
Using 0.172 as the cut point for prediction, Table 5 showed an accuracy of approximately 71% for the derivation set and 62% for the validation set, indicating that the model correctly predicted outcomes in about 60 to 70% of cases, which was not particularly strong. In the derivation set, sensitivity was lower than specificity, suggesting that the model performed worse in identifying individuals achieving smoking abstinence but had better specificity, reflecting stronger performance in correctly predicting outcomes for those not achieving smoking abstinence. Conversely, the validation set exhibited the opposite pattern, with sensitivity slightly higher than specificity. The Positive Predictive Value (PPV) indicated that 38.5% of those with a positive test in the derivation set actually achieved smoking abstinence, slightly higher than the 33.8% observed in the validation set. The Negative Predictive Value (NPV) showed that 88.6% of those with a negative test in the derivation set did not achieve smoking abstinence, slightly higher than the 85% observed in the validation set.
