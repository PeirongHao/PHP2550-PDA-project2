...
Since adults with major depressive disorder (MDD) often exhibit higher nicotine dependence, this project, in collaboration with Dr. George Papandonatos from Brown University's Department of Biostatistics, investigates smoking cessation in adults with MDD. In a [prior study](https://pubmed.ncbi.nlm.nih.gov/37069490/), Dr. Papandonatos used a 2x2 factorial design to assess the effectiveness of Behavioral Activation for Smoking Cessation (BASC) versus standard behavioral treatment (ST), alongside varenicline versus placebo. Results showed that varenicline significantly improved cessation rates compared to placebo, though no substantial differences were found between BASC and ST in increasing abstinence rates. Building on these findings, our current study will analyze baseline variables to identify potential predictors of smoking abstinence, while adjusting for treatment effects.
...
#| fig-height: 3
md.plot<-md.pattern(df.proj2, plot = TRUE, rotate.names = TRUE)
```

To further determine the type of missingness, I created binary indicators (1 for missing, 0 for not missing) for each variable and generated a correlation plot for missingness (refer to Figure 2). Since many of the correlations had absolute values greater than 0.2, with a maximum of 0.63, I considered the missingness to plausibly be Missing at Random (MAR), meaning the probability of an observation being missing depends only on the observed variables.
...
```{r}
#| tbl-cap: Coefficients of logistic model
...
A positive coefficient of 1.384 means that individuals who engage in Behavioral Activation are more likely to achieve smoking abstinence. Holding other variables constant, Behavioral Activation increases the odds of smoking abstinence by approximately 4 (=$e^{1.384}$) times.

A positive coefficient of 0.066 indicates that there’s a small positive interaction effect between Age and Education on smoking abstinence. This means that the combined effect of age and education slightly increases the odds of smoking abstinence. However, this effect is relatively small compared to the other predictors.

This positive coefficient suggests that there’s an interaction between Age and having Current vs Past Major Depressive Disorder (MDD) that influences smoking abstinence. This implies that, as age increases, individuals with current vs. past MDD have 1.29 (=$e^{0.253}$) times higher odds of achieving smoking abstinence than those with past MDD only.

A negative coefficient of -0.024 indicates that there’s an interaction effect between BDI score (a measure of depression severity) and Readiness to Quit Smoking on the likelihood of smoking abstinence. This suggests that, for individuals with higher BDI scores, even if they are ready to quit, they might have a lower likelihood of achieving smoking abstinence.
The effect size is modest ($e^{-0.024}$=0.98), but it points to a potential barrier for individuals with higher depression symptoms.
...
Behavioral Activation has the largest positive effect, significantly increasing the odds of smoking abstinence. Age and Education and Age with Current vs Past MDD both have positive but smaller effects on smoking abstinence. Higher Depression Symptoms (BDI score) combined with Readiness to Quit Smoking has a slightly negative impact, suggesting that depression might interfere with the ability to quit smoking even among those who are motivated. 


# Summary
This model highlights the importance of Behavioral Activation in promoting smoking abstinence, while also indicating that age, education, mental health status, and readiness to quit have nuanced effects on the likelihood of successfully quitting smoking.
