...
ggplot(df.proj2.revise, aes(x = `Smoking Abstinence`, y = Anhedonia, fill = `Smoking Abstinence`)) +
  ...
  theme_minimal()
```
I checked the boxplots of all quantitative variables against the outcome. Most of the boxplots have a similar range for both outcome group. However, the range of Anhedonia seems to differ a lot between the two outcome groups (see violin plot), suggesting Anhedonia could be an important predictor.
...
I made a correlation plot for all quantitative variables pairs. Overall, the correlations are not strong. Three correlations have an absolute value of correlation greater than 0.5. The strongest correlation, maximum of absolute value of correlation, is 0.625 between FTCD score at baseline and Smoking with 5 mins of waking up. Current vs past MDD and BDI score at baseline have correlation of 0.577. FTCD score at baseline and Cigarettes per day at baseline phone survey have correlation of 0.517. 
...
The data is unbalance, with 79% of 0s and 21% of 1s being the outcome value. From the table comparing baseline variables in two outcome groups, we can identify 4 variables that are significantly different between two outcome groups. `Varenicline`, `Race`, `FTCD score at baseline`, `Nicotine Metabolism Ratio` have p-values <$alpha=0.05$ and can be essential predictors. For each categorical variable except the outcome, I made a two way proportion table for it with the outcome. It is worth pointing out that for those with `Baseline readiness to quit smoking` being 3 or 4, they did not smoke abstinence (outcome = 0). For people with `Baseline readiness to quit smoking` being 5, 31% did smoke abstinence (outcome = 1). For people with higher values of `Baseline readiness to quit smoking`, 14% to 25% did smoke abstinence (outcome = 1).
...
From the *Stacked barplot of outcome by treatment group*, participnats from the Varenicline group had higher proportion of outcome being 1 (Smoking Abstinence). If we look at the placebo groups, `ST` seems to be more effective than `BASC` since `ST` associates with a higher proportion of outcome being 1 (Smoking Abstinence).
